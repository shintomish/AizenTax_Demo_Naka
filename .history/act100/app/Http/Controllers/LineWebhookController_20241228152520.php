    ...
    private function replyNormalQuery($replyToken)
    {
        ...
        $buttonTemplate = new ButtonTemplateBuilder(
            ...
            [
                ...
                new UriTemplateActionBuilder('ボタン2', 'https://www.tax-trial.com/')
            ]
        );

        $templateMessage = new TemplateMessageBuilder('ボタンテンプレートのテスト', $buttonTemplate);

        $buttonTemplate = new ButtonTemplateBuilder(
            'タイトル', // 最大40文字
            '説明文', // 最大160文字
            'https://example.com/sample.jpg', // HTTPS形式の有効な画像URL
            [
                new MessageTemplateActionBuilder('ボタン1', 'アクション1'), // ボタンの表示名とアクション内容
                new UriTemplateActionBuilder('詳細はこちら', 'https://example.com') // ボタンの表示名とURL
            ]
        );
        ...
        if (!$response->isSucceeded()) {
            ...
            // \Log::info('Access Token: ' . env('LINE_MESSAGE_CHANNEL_TOKEN'));
            // \Log::error('HTTP Status: ' . $response->getHTTPStatus());
            // \Log::error('Error Message: ' . $response->getRawBody());
