...
class TopClientController extends Controller
{
    ...
    {
        Log::info('topclient show_file START');

        $disk       = 'local';  // or 's3'
        $storage    = Storage::disk($disk);
        $attachpath = 'public/mail_attachments/' . $file_name;
        $storageath = $storage->get($attachpath);

        Log::info('topclient show_file $file_name = ' . print_r($file_name, true));
        Log::info('topclient show_file END');

        return response($storageath, 200)
