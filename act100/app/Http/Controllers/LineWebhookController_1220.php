...
class LineWebhookController extends Controller
{
                ...
                switch ($event['message']['type']) {
                    case 'text':
                        ...
                        if( $updata['count'] == 0 ) {
                            ...
                            // 商品価格を返信するロジック
                            $msg = $this->getProductPrice($userMessage);
                            // $textMessageBuilder = new TextMessageBuilder($replyText);
            
                            // ユーザーに返信
                            $response = $bot->replyText($event['replyToken'], $msg);
                            Log::info('LineWebhookController message case text $msg = ' . print_r($msg, true));
                        }
    ...
    private function getProductPrice($productName)
    {
        ...
        return $productPrices[$productName] . " です。" ?? '商品の価格が見つかりません。';
    }
}
